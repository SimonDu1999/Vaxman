...
float* monster1 = new float[6] {10.5, 8.5, 1.0, 0.0, 1.0, 1.0}; //coordinates and direction and color of first monster (cyan)
float* monster2 = new float[6] {13.5, 1.5, 2.0, 1.0, 0.0, 0.0}; //coordinates and direction and color of second monster (red)
float* monster3 = new float[6] {4.5, 6.5, 3.0, 1.0, 0.0, 0.6}; //coordinates and direction and color of third monster (magenta)
float* monster4 = new float[6] {2.5, 13.5, 4.0, 1.0, 0.3, 0.0}; //coordinates and direction and color of fourth monster (orange)
float old_t;
vector<float*> monsters;
...
void resetGame(){
	...
	yIncrement = 0; 
	rotation = 0;
	monster1 = new float[6] {10.5, 8.5, 1.0, 0.0, 1.0, 1.0 };
	monster2 = new float[6] {13.5, 1.5, 2.0, 1.0, 0.0, 0.0 };
	monster3 = new float[6] {4.5, 6.5, 3.0, 1.0, 0.0, 0.6 };
	monster4 = new float[6] {2.5, 13.5, 4.0, 1.0, 0.3, 0.0 };
	monsters.push_back(monster1);
	monsters.push_back(monster2);
	monsters.push_back(monster3);
	monsters.push_back(monster4);
...
void gameLogic(){
	if (monsters.size() >= 32 * 4) {
		over = true;
		return;
	}
	int pacmanX = (int)(1.5 + xIncrement);
	int pacmanY = (int)(1.5 + yIncrement);
	vector<float*>::iterator d_monster = monsters.end();
	for (auto it = monsters.begin(); it != monsters.end(); ++it) {
		float *monster = *it;
		int monsterX = (int)(monster[0]);
		int monsterY = (int)(monster[1]);
	...
	if (d_monster != monsters.end()) {
		monsters.erase(d_monster);
	}
...
void infectionGrow(void) {
	...
	int n = monsters.size();
	for (int i = 0; i < n; ++i) {
		float* new_monster = new float[6]{ monsters[i][0], monsters[i][1], monsters[i][2], monsters[i][3], monsters[i][4], monsters[i][5] };
	...
	if (replay){
		if (!over){
			...
			for (auto it = monsters.begin(); it != monsters.end(); ++it) {
				...
				drawMonster(monster[0], monster[1], monster[3], monster[4], monster[5]);
			}
			//updateMonster(monsters[0], 1);
			//updateMonster(monsters[1], 2);
			//updateMonster(monsters[2], 3);
			//updateMonster(monsters[3], 4);
			//drawMonster(monsters[0][0], monsters[0][1], 0.0, 1.0, 1.0); //cyan
			//drawMonster(monsters[1][0], monsters[1][1], 1.0, 0.0, 0.0); //red
			//drawMonster(monsters[2][0], monsters[2][1], 1.0, 0.0, 0.6); //magenta
			//drawMonster(monsters[3][0], monsters[3][1], 1.0, 0.3, 0.0); //orange
		}
		else {
			resultsDisplay();
		}
